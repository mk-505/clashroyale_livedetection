...
def main() -> None:
    parser = argparse.ArgumentParser(description="Live Clash Royale PPO inference + rollout collection")
    ...
    parser.add_argument("--no-op-bias", type=float, default=0.2)
    parser.add_argument("--min-elixir-to-act", type=float, default=2.0)
    parser.add_argument("--min-detections-to-act", type=int, default=3)
    ...
    ppo_cfg = PPOConfig(device="cuda" if torch.cuda.is_available() else "cpu")
    policy, optimizer = load_or_create_model(cfg.checkpoint_path, ppo_cfg)
    policy.eval()

    print(f"YOLO model: {Path(cfg.model_path).resolve()}")
    print(f"YOLO classes: {len(yolo.names)}")
    print(f"Capture: {capture.describe_source()}")
    print("Live inference started. Manual flow: start match yourself, bot plays this single match, then exits.")
    ...
    try:
        while True:
            ...
            if now - last_action_t >= cfg.action_interval_sec:
                with torch.no_grad():
                    act = policy.act(state, no_op_bias=cfg.no_op_bias)
                action = int(act["action"])
                # Safety gating when detector signal is weak.
                if len(detections) < args.min_detections_to_act:
                    action = 0
                if float(features["elixir"]) < args.min_elixir_to_act:
                    action = 0
                ...
                prev_log_prob = float(act["log_prob"])
            ...
            if args.show:
                ...
                if args.debug_state:
                    ...
                    lines = [
                        ...
                        f"action={last_action} hand={features['hand_ids']}",
                    ]
                    y0 = 26
                    for i, line in enumerate(lines):
                        y = y0 + i * 24
                        cv2.putText(annotated, line, (12, y), cv2.FONT_HERSHEY_SIMPLEX, 0.65, (20, 255, 20), 2)
                cv2.imshow("CR Live RL Inference", annotated)
    ...
    print("Run train_ppo.py before starting the next match.")
